...
type Brand = {
  ...
  max_display_cap: number | null; product_count: number; ready_count: number; unmatched_count: number;
};
...
export function BrandsControl() {
  ...
  async function update(id: string, changes: { enabled?: boolean; maxDisplayCap?: number | null }) {
    setSaving(id);
    try {
  ...
  return (
    <div className="stack">
      ...
      <div className="card">
        ...
        : <div className="table-wrap"><table className="data-table"><thead><tr><th>Brand</th><th>Coverage</th><th>Display cap</th><th>Enabled</th><th>Actions</th></tr></thead><tbody>
          {visible.map((brand) => {
